...
pub fn unrolled_states_from_Z3_ast<'ctx, T: Ast<'ctx>>(ast: &T, variables: &HashSet<&'ctx str>, k: usize) -> Vec<EnvState<'ctx>> {
    // Create variable mapping
    let mut states: Vec<EnvState<'ctx>> = (0..=k)
        .map(|_| IndexMap::new())
        .collect();
    extract_variables_rec(ast, variables, &mut states);
    states
}
...
fn extract_variables_rec<'ctx, T: Ast<'ctx>>(
    ...
    if let Some(name_ref) = variables.get(name.as_str()) {
        let state_name = ast.children()[0].decl().name();
    ...
    for child in ast.children() {
        extract_variables_rec(&child, variables, states);
    }
}
