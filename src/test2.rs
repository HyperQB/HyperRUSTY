...
pub fn unroll_hltl_formula<'ctx>(ctx: &'ctx Context, formula: &AstNode, paths: &Vec<&Vec<EnvState<'ctx>>>, sem: &Semantics) -> Bool<'ctx> {
    ...
    let new_formula = remove_quantifiers(formula);
    unroll_ltl_formula(ctx, new_formula, paths, &mapping, 0, sem).unwrap_bool()
}

fn remove_quantifiers(formula: &AstNode) -> &AstNode {
    match formula {
        AstNode::HAQuantifier{identifier: _, form} |
        AstNode::HEQuantifier{identifier: _, form} => {
            remove_quantifiers(form)
        }
        _ => formula
    }
}
